# Broken Fixtures

## Overview
These fixtures are intentionally invalid. They exist to exercise validation failures and should not be rendered as gallery examples.

## What It Shows
- Validation errors for missing required fields or unknown registry references
- Negative test cases for the CLI and renderer validation pipeline

## Files
- [`missing-subtitle.ir.json`](./missing-subtitle.ir.json)
- [`unknown-registry-id.ir.json`](./unknown-registry-id.ir.json)

## Regenerate
These files are intentionally broken and should not be rendered.
