...
- Enforce required answers inside the screen that owns them, before allowing navigation to advance: a `kind: "text"` descriptor cannot be submitted empty, a required item list cannot close before its first item, and External Systems needs both its first cluster and at least one item in every cluster. Keep the final `assemble()`/`validate()` pass as defense in depth, not as the normal place authors discover an incomplete section; see [issue #91](https://github.com/ayeshLK/archsmith/issues/91) and [PR #100](https://github.com/ayeshLK/archsmith/pull/100).
- Persist output choices in the IR whenever rerendering must reproduce them. The Legend screen's Include/Omit answer is why `legend` itself is optional in schema 0.3.4; a wizard-only render flag would make the saved JSON produce a different SVG when passed to `archsmith render` later (issue #101).
