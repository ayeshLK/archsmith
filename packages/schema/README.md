...
- `diagram-schema.json` — JSON Schema for the IR (intermediate representation): the structured document an LLM should produce from a rough sketch/text description, and that a future renderer consumes to emit SVG. Fixed shape: 5 columns, Core Platform sub-layers stacked in order, systems-of-record as a sibling section below the "deployed in" wrapper.
