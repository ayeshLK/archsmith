...
- All six step types ship together in v1 — `loop`/`break`/`parallel` were originally proposed as a fast-follow, then pulled forward once their design showed the incremental cost was low (both reuse `decision`'s side-by-side layout or an ordinary sequential stack).
